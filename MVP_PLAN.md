# LuloCustoms Shop - MVP Plan

## STATUS: 🚧 GOTOWY DO IMPLEMENTACJI (5% UKOŃCZONE)

✅ Konfiguracja projektu zakończona
⏳ Oczekuje na implementację 23 kroków

---

## Architektura (SKORYGOWANA)

- **Frontend**: Next.js 16 (Static Export - SSG)
- **Backend**: PHP 8.0+ (API REST)
- **Baza danych**: MySQL 8.0+
- **Płatności**: Przelewy24 (Blik + przelewy)
- **Hosting**: Static files (HTML/CSS/JS) + PHP + MySQL

### Dlaczego ta architektura?
✅ **Hosting wspiera**: Tylko statyczne pliki + PHP + MySQL
❌ **Hosting NIE wspiera**: Node.js runtime/SSR
✅ **Rozwiązanie**: Next.js generuje statyczne pliki → upload przez FTP

---

## Struktura na serwerze

```
sklep.lulocustoms.pl/
├── out/                   # Next.js static export (build lokalnie → upload FTP)
│   ├── index.html
│   ├── produkty/
│   ├── koszyk/
│   ├── zamowienie/
│   ├── admin/
│   └── _next/            # Statyczne assety (CSS, JS)
│
├── api/                   # PHP Backend (upload przez FTP)
│   ├── config.php        # Konfiguracja (DB, Przelewy24)
│   ├── database.php      # Połączenie z MySQL
│   ├── auth.php          # Logowanie admina
│   ├── products.php      # CRUD produktów
│   ├── orders.php        # Zamówienia
│   └── payment.php       # Przelewy24 webhook
│
├── uploads/              # Zdjęcia produktów (upload przez admin)
│   └── products/
│
└── .htaccess            # Routing (CORS, security headers)
```

---

## Struktura projektu (lokalna)

```
lulocustoms-shop/
├── src/
│   ├── app/                    # Next.js Pages (SSG)
│   │   ├── page.tsx           # Strona główna - lista produktów
│   │   ├── produkty/
│   │   │   └── [id]/
│   │   │       └── page.tsx   # Szczegóły produktu
│   │   ├── koszyk/
│   │   │   └── page.tsx       # Koszyk
│   │   ├── zamowienie/
│   │   │   └── page.tsx       # Checkout
│   │   ├── potwierdzenie/
│   │   │   └── page.tsx       # Potwierdzenie po płatności
│   │   └── admin/
│   │       ├── page.tsx       # Logowanie
│   │       ├── produkty/
│   │       │   └── page.tsx   # Zarządzanie produktami
│   │       └── zamowienia/
│   │           └── page.tsx   # Lista zamówień
│   │
│   ├── components/            # Komponenty React
│   │   ├── ui/               # Button, Input, Card, Modal
│   │   ├── shop/             # ProductCard, CartItem, CheckoutForm
│   │   └── admin/            # ProductForm, OrderTable, LoginForm
│   │
│   ├── lib/                  # Utilities (tylko frontend!)
│   │   ├── api.ts           # Funkcje do wywołań PHP API
│   │   └── cart.ts          # localStorage cart logic
│   │
│   └── types/               # TypeScript types
│       └── index.ts
│
├── api/                      # PHP Backend (standalone)
│   ├── config.php
│   ├── database.php
│   ├── auth.php
│   ├── products.php
│   ├── orders.php
│   └── payment.php
│
├── database/                 # SQL migrations
│   ├── schema.sql           # Pełny schemat bazy
│   └── seed.sql             # Przykładowe dane (opcjonalnie)
│
├── public/                   # Pliki statyczne
│   └── images/
│
└── out/                      # Generated by `npm run build` (upload to FTP)
```

---

## Funkcjonalności MVP

### 1. SKLEP (dla klientów)
- [ ] Lista wszystkich produktów (SSG - generowane podczas build)
- [ ] Szczegóły produktu (zdjęcie, opis, cena)
- [ ] Koszyk (localStorage - działa offline)
- [ ] Checkout (formularz: imię, email, telefon, adres)
- [ ] Płatność przez Przelewy24 (Blik + przelewy bankowe)
- [ ] Strona potwierdzenia zamówienia

### 2. PANEL ADMINA
- [ ] Logowanie (email + hasło → PHP session)
- [ ] Lista produktów (pobierana z API PHP)
- [ ] Dodawanie produktu (nazwa, opis, cena, upload zdjęcia)
- [ ] Edycja produktu
- [ ] Usuwanie produktu
- [ ] Lista zamówień (status płatności, dane klienta, produkty)

### 3. BAZA DANYCH (4 tabele)

```sql
-- Tabela 1: Administratorzy
admin_users:
- id (INT, PRIMARY KEY, AUTO_INCREMENT)
- email (VARCHAR(255), UNIQUE)
- password_hash (VARCHAR(255))
- created_at (DATETIME)

-- Tabela 2: Produkty
products:
- id (INT, PRIMARY KEY, AUTO_INCREMENT)
- name (VARCHAR(255))
- description (TEXT)
- price (DECIMAL(10,2))
- image_url (VARCHAR(500))
- stock (INT, DEFAULT 0)
- active (BOOLEAN, DEFAULT true)
- created_at (DATETIME)
- updated_at (DATETIME)

-- Tabela 3: Zamówienia
orders:
- id (INT, PRIMARY KEY, AUTO_INCREMENT)
- order_number (VARCHAR(50), UNIQUE)
- customer_name (VARCHAR(255))
- customer_email (VARCHAR(255))
- customer_phone (VARCHAR(20))
- customer_address (TEXT)
- total_price (DECIMAL(10,2))
- payment_status (ENUM: 'pending', 'paid', 'failed')
- p24_transaction_id (VARCHAR(100))
- created_at (DATETIME)

-- Tabela 4: Pozycje zamówienia
order_items:
- id (INT, PRIMARY KEY, AUTO_INCREMENT)
- order_id (INT, FOREIGN KEY → orders.id)
- product_id (INT)
- product_name (VARCHAR(255))  -- snapshot na wypadek usunięcia produktu
- product_price (DECIMAL(10,2)) -- snapshot ceny z momentu zakupu
- quantity (INT)
```

### 4. BEZPIECZEŃSTWO

- [ ] **SQL Injection**: Prepared statements w PHP (PDO)
- [ ] **Password**: bcrypt hashing (PHP password_hash)
- [ ] **Session**: Secure PHP sessions (httponly, secure, samesite)
- [ ] **XSS**: Input validation + htmlspecialchars w PHP
- [ ] **CSRF**: Token validation dla POST requestów
- [ ] **CORS**: .htaccess headers (tylko sklep.lulocustoms.pl)
- [ ] **HTTPS**: Force HTTPS w .htaccess
- [ ] **File Upload**: Walidacja typu/rozmiaru zdjęć
- [ ] **Rate Limiting**: Ochrona przed brute force (login, API)

---

## Czego NIE MA w MVP (v2.0+)

- Rejestracja klientów (kupują jako goście)
- Konto klienta / historia zamówień
- Kategorie produktów
- Filtry i wyszukiwarka
- Warianty produktów (rozmiary, kolory)
- Recenzje produktów
- Newsletter
- Kody rabatowe
- Śledzenie przesyłek
- Zaawansowane statystyki
- Multi-language

---

## Plan realizacji (23 kroki)

### ETAP 1: BAZA DANYCH (3 kroki)
1. [ ] **Utworzyć schemat bazy MySQL** (`database/schema.sql`)
   - 4 tabele: admin_users, products, orders, order_items
   - Indeksy, foreign keys, defaults
2. [ ] **Napisać skrypt seed** (`database/seed.sql`)
   - Pierwsze konto admina (email + hasło)
   - 3-5 przykładowych produktów do testów
3. [ ] **Przygotować dokumentację setup bazy**
   - Instrukcje import SQL przez phpMyAdmin
   - Jak zmienić pierwsze hasło admina

### ETAP 2: PHP BACKEND (7 kroków)
4. [ ] **config.php** - Konfiguracja
   - Dane połączenia MySQL (z phpMyAdmin)
   - Klucze Przelewy24 (sandbox mode)
   - Security settings (session, CORS)
5. [ ] **database.php** - Połączenie z bazą
   - PDO connection z error handling
   - Prepared statements helper functions
6. [ ] **auth.php** - Autentykacja admina
   - POST /api/auth.php?action=login
   - POST /api/auth.php?action=logout
   - GET /api/auth.php?action=check (czy zalogowany)
7. [ ] **products.php** - CRUD produktów
   - GET /api/products.php (lista wszystkich)
   - GET /api/products.php?id=X (pojedynczy)
   - POST /api/products.php (dodaj - tylko admin)
   - PUT /api/products.php (edytuj - tylko admin)
   - DELETE /api/products.php?id=X (usuń - tylko admin)
   - Upload zdjęć do /uploads/products/
8. [ ] **orders.php** - Zamówienia
   - POST /api/orders.php (utwórz zamówienie)
   - GET /api/orders.php (lista - tylko admin)
   - GET /api/orders.php?id=X (szczegóły - tylko admin)
9. [ ] **payment.php** - Przelewy24
   - POST /api/payment.php?action=init (inicjalizacja płatności)
   - POST /api/payment.php?action=verify (webhook P24)
   - POST /api/payment.php?action=status (sprawdź status)
10. [ ] **.htaccess** - Security & Routing
    - Force HTTPS
    - CORS headers
    - Zabezpieczenia (block .env, .git, etc.)

### ETAP 3: FRONTEND - KOMPONENTY UI (2 kroki)
11. [ ] **Podstawowe komponenty** (`src/components/ui/`)
    - Button.tsx (primary, secondary, danger)
    - Input.tsx (text, email, number)
    - Card.tsx (kontener produktu)
    - Modal.tsx (dialogi)
    - Spinner.tsx (loading state)
12. [ ] **TypeScript types** (`src/types/index.ts`)
    - Product, Order, OrderItem, Admin
    - API Response types
    - Cart types (localStorage)

### ETAP 4: FRONTEND - SKLEP (5 kroków)
13. [ ] **Strona główna** (`src/app/page.tsx`)
    - Fetch produktów z API PHP
    - Grid produktów (ProductCard)
    - "Dodaj do koszyka" button
14. [ ] **Szczegóły produktu** (`src/app/produkty/[id]/page.tsx`)
    - Fetch pojedynczego produktu
    - Galeria zdjęć
    - Opis, cena, stock
    - "Dodaj do koszyka"
15. [ ] **Koszyk** (`src/app/koszyk/page.tsx`)
    - Odczyt z localStorage
    - Lista produktów w koszyku
    - Zmiana ilości, usuwanie
    - Suma zamówienia
    - Przycisk "Przejdź do kasy"
16. [ ] **Checkout** (`src/app/zamowienie/page.tsx`)
    - Formularz: imię, email, telefon, adres
    - Walidacja (react-hook-form + zod)
    - Podsumowanie zamówienia
    - Inicjalizacja płatności P24
    - Redirect do Przelewy24
17. [ ] **Potwierdzenie** (`src/app/potwierdzenie/page.tsx`)
    - Weryfikacja płatności (query params z P24)
    - Wyświetl potwierdzenie lub błąd
    - Wysyłka email (opcjonalnie)

### ETAP 5: FRONTEND - PANEL ADMINA (3 kroki)
18. [ ] **Logowanie** (`src/app/admin/page.tsx`)
    - Formularz email + hasło
    - Wywołanie PHP auth.php
    - Zapisanie sesji (localStorage token lub cookie)
    - Redirect do /admin/produkty
19. [ ] **Zarządzanie produktami** (`src/app/admin/produkty/page.tsx`)
    - Sprawdzenie czy zalogowany (middleware?)
    - Lista produktów (tabela)
    - Modal dodawania/edycji
    - Upload zdjęcia (FormData)
    - Usuwanie (confirmation dialog)
20. [ ] **Lista zamówień** (`src/app/admin/zamowienia/page.tsx`)
    - Tabela zamówień
    - Filtry: status, data
    - Podgląd szczegółów (modal)
    - Zmiana statusu (opcjonalnie)

### ETAP 6: FINALIZACJA (3 kroki)
21. [ ] **Walidacja i zabezpieczenia**
    - Testy wszystkich API endpoints
    - Sprawdzenie XSS/CSRF/SQL injection
    - Rate limiting na login
    - Walidacja uploadowanych plików
22. [ ] **Dokumentacja deployment**
    - README: jak zbudować lokalnie
    - Instrukcje FTP upload
    - Setup bazy danych
    - Konfiguracja Przelewy24
    - Zmienne środowiskowe (.htaccess)
23. [ ] **Testing & Deploy**
    - Test pełnego flow: przeglądanie → koszyk → płatność
    - Test panelu admina
    - Upload na hosting przez FTP
    - Test na produkcji

---

## Deployment

### 1. Lokalne budowanie:

```bash
# Instalacja dependencies
npm install

# Development (z hot reload)
npm run dev

# Production build (generuje statyczne pliki)
npm run build
# ↑ Tworzy folder /out z HTML/CSS/JS
```

### 2. Upload na hosting (FTP):

**Co uploadować:**
1. **Zawartość folderu `out/`** → główny katalog domeny (public_html lub httpdocs)
2. **Folder `api/`** → obok `out/` (na tym samym poziomie)
3. **Folder `uploads/`** → obok `api/` (z prawami zapisu 755)
4. **Plik `.htaccess`** → główny katalog

**Struktura końcowa na FTP:**
```
public_html/
├── index.html           # z out/
├── produkty/            # z out/
├── _next/               # z out/
├── api/
│   ├── config.php
│   ├── database.php
│   └── ...
├── uploads/
│   └── products/
└── .htaccess
```

### 3. Konfiguracja bazy MySQL:

1. Wejdź do phpMyAdmin (panel hostingu)
2. Utwórz nową bazę: `lulocustoms_shop`
3. Import pliku: `database/schema.sql`
4. (Opcjonalnie) Import: `database/seed.sql`
5. Zapisz dane dostępowe (host, user, password, nazwa bazy)

### 4. Konfiguracja PHP:

Edytuj `api/config.php` na serwerze:

```php
// Dane MySQL (z phpMyAdmin)
define('DB_HOST', 'localhost');
define('DB_NAME', 'lulocustoms_shop');
define('DB_USER', 'twoj_user');
define('DB_PASS', 'twoje_haslo');

// Przelewy24 (sandbox → potem production)
define('P24_MERCHANT_ID', 'xxxxx');
define('P24_POS_ID', 'xxxxx');
define('P24_CRC', 'xxxxxxxx');
define('P24_TEST_MODE', true);  // false na produkcji
```

### 5. Konfiguracja Przelewy24:

1. Zarejestruj konto na https://przelewy24.pl
2. Przejdź do panelu → **Moje dane** → **Sklepy i Stanowiska**
3. Uzyskaj:
   - **Merchant ID** (ID Sprzedawcy)
   - **POS ID** (ID Stanowiska)
   - **CRC** (Klucz do raportów)
   - **API Key** (opcjonalnie, dla API)
4. Ustaw **URL powrotu**: `https://sklep.lulocustoms.pl/potwierdzenie`
5. Ustaw **Webhook URL**: `https://sklep.lulocustoms.pl/api/payment.php?action=verify`
6. Najpierw testuj w **trybie sandbox** (P24_TEST_MODE = true)
7. Po testach → aktywuj konto → zmień na tryb produkcyjny

### 6. Pierwsze logowanie:

```
Email: admin@lulocustoms.pl
Hasło: (hasło z seed.sql - ZMIEŃ natychmiast po pierwszym logowaniu!)
```

---

## Wymagania techniczne

**Na hostingu:**
- PHP 8.0+
- MySQL 8.0+
- mod_rewrite (Apache)
- HTTPS (SSL certificate)
- min 100MB miejsca
- phpMyAdmin (do zarządzania bazą)

**Lokalnie (development):**
- Node.js 18+
- npm lub yarn
- Edytor kodu (VS Code)

---

## Bezpieczeństwo - Checklist

- [ ] Hasła hashowane przez `password_hash()` (bcrypt)
- [ ] Prepared statements (PDO) - zero konkatenacji SQL
- [ ] Session security (httponly, secure, samesite)
- [ ] CSRF tokens na wszystkich formularzach
- [ ] XSS protection (`htmlspecialchars()` w PHP)
- [ ] Input validation (PHP + Zod frontend)
- [ ] File upload validation (typ, rozmiar, rozszerzenie)
- [ ] Rate limiting na login endpoint
- [ ] HTTPS only (redirect w .htaccess)
- [ ] Blocked access do .env, .git, config files
- [ ] CORS tylko dla sklep.lulocustoms.pl
- [ ] Secrets w config.php poza document root (jeśli możliwe)

---

## Czas realizacji

**Szacowany czas**: 1-2 dni robocze dla doświadczonego dewelopera
**MVP**: Wszystkie 23 kroki można wykonać w 2-3 sesjach po 4-6h

**Priorytet implementacji:**
1. Backend + Baza (kroki 1-10) → 40% czasu
2. Frontend sklep (kroki 11-17) → 35% czasu
3. Panel admin (kroki 18-20) → 15% czasu
4. Finalizacja (kroki 21-23) → 10% czasu

---

## Następne kroki (v2.0)

Po ukończeniu MVP, w kolejnych wersjach:
- Konta użytkowników
- Historia zamówień dla klientów
- Kategorie i filtry produktów
- Warianty produktów (rozmiary, kolory)
- System recenzji
- Newsletter (Mailchimp/SendGrid)
- Kody rabatowe
- Śledzenie przesyłek (integracja z kurierami)
- Statystyki sprzedaży (dashboard admina)
- Multi-language (PL/EN)
- PWA (Progressive Web App)
